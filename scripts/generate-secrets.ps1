...
Write-Host ""
Write-Host "Generate and upload DASHBOARD_PASSWORD_HASH after npm install:"
Write-Host "`$hash = node ./scripts/hash-password.mjs `"$dashboardPassword`""
Write-Host "npx supabase secrets set DASHBOARD_PASSWORD_HASH=`"`$hash`""
Write-Host ""
